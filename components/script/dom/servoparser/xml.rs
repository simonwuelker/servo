...
impl Tokenizer {
    ...
    pub(crate) fn parse(&self) -> impl Iterator<Item = ParserAction<DomRoot<HTMLScriptElement>>> {
        self.inner.parse().flat_map(|action| {
    ...
    pub(crate) fn reset_insertion_point_to_start_of_stream(&self) {
        self.inner
            .input_stream()
            .reset_insertion_point_to_start_of_stream();
    }
    ...
    pub(crate) fn insert_code_points_at_insertion_point(&self, chunk: StrTendril) {
        self.inner.input_stream().write_at_insertion_point(chunk);
    }
}
