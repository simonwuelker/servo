...
impl EncodingDetector {
    ...
    fn buffer(&mut self, data: &[u8]) -> Option<(&'static Encoding, EncodingConfidence)> {
        ...
        self.determine_the_character_encoding(true)
    }
    ...
    fn determine_the_character_encoding(
        ...
        if !self.attempted_bom_sniffing && self.buffered_bytes.len() > 2 {
            ...
            match self.buffered_bytes.as_slice() {
                &[0xEF, 0xBB, 0xBF, ..] => return Some((UTF_8, EncodingConfidence::Certain)),
                &[0xFE, 0xFF, ..] => return Some((UTF_16BE, EncodingConfidence::Certain)),
                &[0xFF, 0xFE, ..] => return Some((UTF_16LE, EncodingConfidence::Certain)),
                _ => {},
            }
        }
        ...
        if let Some(encoding) =
            prescan_the_byte_stream_to_determine_the_encoding(&self.buffered_bytes)
        {
...
impl NetworkDecoderState {
    pub(super) fn new() -> Self {
        Self::Buffering(EncodingDetector::default())
    }
...
fn extract_a_character_encoding_from_a_meta_element(input: &[u8]) -> Option<&'static Encoding> {
    ...
    loop {
        ...
        position += input[position..]
            .windows(7)
            .position(|window| window == b"charset")? + b"charset".len();
