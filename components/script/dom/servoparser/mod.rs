...
use markup5ever::buffer_queue::ByteBufferQueue;
use mime::{self, Mime};
...
impl ServoParser {
    ...
    pub(crate) fn resume_with_pending_parsing_blocking_script(
        ...
        self.tokenizer.reset_insertion_point_to_start_of_stream();
    ...
    pub(crate) fn write(&self, text: Vec<DOMString>, can_gc: CanGc) {
        ...
        if self.document.has_pending_parsing_blocking_script() {
            ...
            for chunk in text {
                self.tokenizer
                    .insert_code_points_at_insertion_point(String::from(chunk).into());
            }
            return;
        }
        ...
        self.tokenizer
            .parse_complete(|parser_action| self.handle_parsing_action(parser_action, can_gc));
        ...
        if self.suspended.get() {
            ...
            //     self.tokenizer.insert_code_points_at_insertion_point(chunk);
            // }
            return;
        }
    }
    ...
    fn parse_sync(&self, can_gc: CanGc) {
        ...
        if self.last_chunk_received.get() && !self.last_chunk_processed.get() {
            self.tokenizer.finish_decoding_input();
        ...
        self.tokenizer
            .parse_complete(|action| self.handle_parsing_action(action, can_gc));
    ...
    fn handle_parsing_action(&self, parsing_action: ParserAction, can_gc: CanGc) -> bool {
        ...
        let script = match parsing_action {
            ...
            ParserAction::StartOverWithEncoding(encoding) => {
                log::error!("FIXME: Restart the parser with encoding {encoding:?}");
                return true;
            },
        };
...
impl Tokenizer {
    fn insert_code_points_at_insertion_point(&self, chunk: StrTendril) {
        match self {
            Self::Html(html_tokenizer) => {
                html_tokenizer.insert_code_points_at_insertion_point(chunk)
            },
            Self::AsyncHtml(async_html_tokenizer) => todo!(),
            Self::Xml(xml_tokenizer) => xml_tokenizer.insert_code_points_at_insertion_point(chunk),
        }
    }
    ...
    fn finish_decoding_input(&self) {
        match self {
            Self::Html(html_tokenizer) => html_tokenizer.finish_decoding_input(),
            Self::AsyncHtml(_) => todo!(),
            Self::Xml(xml_tokenizer) => xml_tokenizer.finish_decoding_input(),
        }
    }

    /// Move the insertion point to the beginning of the input stream
    fn reset_insertion_point_to_start_of_stream(&self) {
        match self {
            Self::Html(html_tokenizer) => html_tokenizer.reset_insertion_point_to_start_of_stream(),
            Self::AsyncHtml(_) => todo!(),
            Self::Xml(xml_tokenizer) => xml_tokenizer.reset_insertion_point_to_start_of_stream(),
        }
    }
    /// Run the parser until all input has been consumed.
    fn parse_complete<F>(&self, callback: F)
    where
        F: Fn(ParserAction) -> bool,
    {
        match self {
            Tokenizer::Html(html_tokenizer) => {
                for action in html_tokenizer.parse() {
                    if !callback(action) {
                        break;
                    }
                }
            ...
            Tokenizer::Xml(xml_tokenizer) => {
                for action in xml_tokenizer.parse() {
                    if !callback(action) {
                        break;
                    }
                }
    ...
    fn feed_bytes(&self, bytes: &[u8]) {
        match self {
            Self::Html(html_tokenizer) => html_tokenizer.feed_bytes(bytes),
            Self::AsyncHtml(_) => todo!(),
            Self::Xml(xml_tokenizer) => xml_tokenizer.feed_bytes(bytes),
        }
    }
