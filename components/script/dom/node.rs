...
impl Node {
    ...
    pub(crate) fn is_inclusive_ancestor_of(&self, node: &Node) -> bool {
        self == node || self.is_ancestor_of(node)
    }

    pub(crate) fn is_ancestor_of(&self, node: &Node) -> bool {
        node.ancestors().any(|ancestor| &*ancestor == self)
    }
