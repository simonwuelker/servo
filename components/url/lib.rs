...
impl ServoUrlWithPotentialUnresolvedBlobReference {
    ...
    where F: FnOnce(Uuid, String) -> Result<BlobUrlEntry, String> {
        let url_blob_entry = if self.url.scheme() == "blob" {
            let Some(mut segments) = self.url.path_segments() else {
                return Err(format!("Blob url without path segments: {:?}", self.url.to_string()));
            };

            let Some(id) = segments.next().map(Uuid::from_str).transpose().ok().flatten() else {
                return Err(format!("Blob url with zero segments: {:?}", self.url.to_string()));
            };

            if segments.next().is_some() {
                return Err(format!("Blob url more than one path segment: {:?}", self.url.to_string()));
            }

            let origin = self.url.origin().ascii_serialization();

            Some(Box::new(f(id, origin)?))
        } else {
            None
        };
...
pub struct ServoUrl{
    ...
    url_blob_entry: Option<Box<BlobUrlEntry>>,
}
