...
pub struct GridFormattingContext {
    ...
    children: Vec<ArcRefCell<GridItem>>,
...
impl GridItem {
    ...
    fn is_auto_positioned(&self) -> bool {
        ...
        let row_position_is_auto = style.clone_grid_row_start().is_auto() && style.clone_grid_row_end().is_auto();
        let column_position_is_auto = style.clone_grid_column_start().is_auto() && style.clone_grid_column_end().is_auto();
...
impl GridFormattingContext {
    fn new(style: Arc<ComputedValues>, children: Vec<ArcRefCell<GridItem>>) -> Self {
