...
use super::{GridFormattingContext, GridItem, OccupationGrid};
...
impl GridFormattingContext {
    ...
    pub fn layout(
        ...
        for grid_item in &self.children {
            // FIXME: this item might not have an automatic grid position
            state.place_element_with_automatic_grid_position_in_both_axes(
                grid_item.clone(),
                dimension,
            );
        }
        ...
        layout_context.compute_track_sizes(GridDimension::Column);
        layout_context.compute_track_sizes(GridDimension::Row);
...
struct GridPlacementContext<'fc> {
    ...
    placed_grid_items: Vec<GridItemWithPlacement>,
}
...
impl<'fc> GridPlacementContext<'fc> {
    ...
    fn place_grid_items(&mut self, items: Vec<ArcRefCell<GridItem>>) {
        ...
        for item in &items {
            if item.borrow().is_auto_positioned() {
                continue;
            }
        }
    }

    fn place_element(&mut self, item: ArcRefCell<GridItem>, area: GridArea) {
        log::debug!(target: "grid-layout", "placing grid item at {area:?}");
        self.occupation_grid.mark_area_as_occupied(area);
        let grid_item_with_placement = GridItemWithPlacement {
            item,
            placement: area,
        };
    ...
    fn finish(self) -> Vec<GridItemWithPlacement> {
        self.placed_grid_items
    }

    // fn place_element_with_definite_column_and_definite_row(
    //     &mut self,
    //     formatting_context: &GridFormattingContext,
    //     item: ArcRefCell<GridItem>,
    // ) {
    //     let row_placement = self.compute_grid_placement(item.borrow().style(), GridDimension::Row);
    //     let column_placement =
    //         self.compute_grid_placement(item.borrow().style(), GridDimension::Column);

    //     let area = GridArea {
    //         row_start: row_placement.start,
    //         row_end: row_placement.end,
    //         column_start: column_placement.start,
    //         column_end: column_placement.end,
    //     };

    //     self.place_element(item, area);
    // }

    // /// Compute the [PlacementPosition] for a given grid item.
    // fn compute_grid_placement(
    //     &self,
    //     style: &ComputedValues,
    //     dimension: GridDimension,
    // ) -> PlacementAlongAxis {
    //     // https://github.com/LadybirdBrowser/ladybird/blob/ebe89a32073a1b838f2d1e04d2146f2dfae6b6f7/Userland/Libraries/LibWeb/Layout/GridFormattingContext.cpp#L216
    //     let (placement_start, placement_end) = if dimension == GridDimension::Row {
    //         (
    //             style.clone_grid_column_start(),
    //             style.clone_grid_column_end(),
    //         )
    //     } else {
    //         (style.clone_grid_row_start(), style.clone_grid_row_end())
    //     };

    //     let mut position = PlacementAlongAxis {
    //         start: 0,
    //         end: 0,
    //         span: 1,
    //     };
    ...
    //         if placement_start.line_num.is_positive() {
    //             position.start = placement_start.line_num;
    //         } else if placement_start.line_num.is_negative() {
    //             let line_count = if dimension == GridDimension::Row {
    //                 self.formatting_context.explicit_row_count
    //             } else {
    //                 self.formatting_context.explicit_column_count
    //             };
    //             position.start = line_count + placement_start.line_num;
    //         }
    //     }

    //     if !placement_end.is_span && placement_end.line_num != 0 {
    //         position.end = placement_end.line_num - 1;
    //     }

    //     if position.end.is_negative() {
    //         if dimension == GridDimension::Row {
    //             position.end = self.occupation_grid.row_count() + 2;
    //         } else {
    //             position.end = self.occupation_grid.column_count() + 2;
    //         }
    //     }

    //     // FIXME handle spans, named placements
...
fn find_the_size_of_an_fr(tracks: &[GridTrack], space_to_fill: Au) -> Au {
    ...
    // Step 2. Let flex factor sum be the sum of the flex factors of the flexible tracks. If this value is less than 1, set it to 1 instead.
...
impl<'a> GridLayoutContext<'a> {
    ...
    fn compute_track_sizes(&mut self, dimension: GridDimension) {
    ...
    fn for_each_track_spanned_by_item<F>(
        &self,
        item: &GridItemWithPlacement,
        dimension: GridDimension,
        mut f: F,
    ) where
        F: FnMut(&GridTrack),
    {
        ...
        let span = item.span(dimension);
        let start = item.start(dimension);
    ...
    fn containing_block_size_for_item(
        &self,
        item: &GridItemWithPlacement,
        dimension: GridDimension,
    ) -> Au {
        let mut total_space = Au(0);
        self.for_each_track_spanned_by_item(item, dimension, |track| {
    ...
    /// This should only be called after the track sizes along this axis have been computed
    fn resolve_box_sizing_for_item_along_axis(&self, item: &mut GridItemWithPlacement) {
        let available_width = self.containing_block_size_for_item(item, GridDimension::Column);
    }
}

struct GridItemWithPlacement {
    item: ArcRefCell<GridItem>,
    placement: GridArea,
}

impl GridItemWithPlacement {
    fn span(&self, dimension: GridDimension) -> i32 {
        match dimension {
            GridDimension::Row => self.placement.row_end - self.placement.row_start,
            GridDimension::Column => self.placement.column_end - self.placement.column_start,
        }
    }

    fn start(&self, dimension: GridDimension) -> i32 {
        match dimension {
            GridDimension::Row => self.placement.row_start,
            GridDimension::Column => self.placement.column_start,
        }
    }
}
