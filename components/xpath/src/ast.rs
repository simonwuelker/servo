...
#[derive(Clone, Debug, MallocSizeOf, PartialEq)]
pub(crate) enum NodeTestCondition {
    Name(QualName),
    Wildcard,
...
impl NodeTest {
    pub(crate) const COMMENT: Self = Self::new(NodeTestCondition::Comment);
    pub(crate) const NODE: Self = Self::new(NodeTestCondition::Node);
    pub(crate) const TEXT: Self = Self::new(NodeTestCondition::Text);
    pub(crate) const WILDCARD: Self = Self::new(NodeTestCondition::Wildcard);
    ...
    pub(crate) const fn name(name: QualName) -> Self {
        Self::new(NodeTestCondition::Name(name))
    }

    pub(crate) const fn processing_instruction(target: Option<String>) -> Self {
        Self::new(NodeTestCondition::ProcessingInstruction(target))
    }
}
